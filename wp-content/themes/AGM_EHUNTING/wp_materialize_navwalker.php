...
class wp_materialize_navwalker extends Walker {
  ...
  function end_lvl( &$output, $depth = 0, $args = array() ) {
    ...
    if (empty($item->url) && !empty($children)) {
      $child = $children[0];
      $child_url = get_post_meta($child->ID, '_menu_item_url', true);
      $child_type = get_post_meta($child->ID, '_menu_item_type', true);

      if (empty($child_url) && 'post_type' === $child_type) {
        $child_object_id = get_post_meta($child->ID, '_menu_item_object_id', true);
        $child_url = $child_object_id ? get_permalink($child_object_id) : '';
      }
    ...
    $li_class_names = trim($depth_class_names . ' ' . esc_attr($class_names));
    $output     .= $indent . '<li id="nav-menu-item-'. $item->ID . '" class="' . $li_class_names . '">';
    $attributes  = ! empty( $item->attr_title ) ? ' title="'  . esc_attr( $item->attr_title ) .'"' : '';
    $attributes .= ! empty( $item->target )     ? ' target="' . esc_attr( $item->target     ) .'"' : '';
    $attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
    $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
    $attributes .= ! empty( $children )         ? ' data-activates="dropdown-'. $item->ID .'"' : '';
    $attributes .= ! empty( $children )         ? ' data-beloworigin="true"' : '';
    ...
    if(!empty($children))
      $item_output .= '<i class="arrowdown right"></i>';
